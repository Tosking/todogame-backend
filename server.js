      ...
      if (result.rows.length == 0) {
        pg.query(
          ...
          (err, result) => {
            ...
            const refreshToken = token.generateRefreshToken(result.rows[0].id, result.rows[0].login);
            const accessToken = token.generateAccessToken(result.rows[0].id, result.rows[0].login);
